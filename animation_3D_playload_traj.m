% --- START OF FILE animation_3D_playload_traj.m ---
function animation_3D_playload_traj(t, eta_traj, p)
% animation_3D_playload_traj Animates the 3D mission, highlighting the 
% trajectory (tail) of the payload.
%
...
% emphasizes the path taken by the payload by rendering a trailing line.

%% --- 1. Figure and Axis Setup ---
figure('Name', '3D Animation: Payload Trajectory', 'Position', [100 100 1000 800]);
ax = gca;
...
% Payload marker, its tail, and the title
h_payload = plot3(NaN, NaN, NaN, 'ks', 'MarkerSize', 10, ...
    'MarkerFaceColor', 'r', 'DisplayName', 'Payload');
h_payload_tail = plot3(NaN, NaN, NaN, 'k-', 'LineWidth', 2, ...
...
video_filename = 'mission_animation_p.mp4';
...
for ti = 1:animation_step:length(t)
    ...
    % Update the payload's trajectory tail
    tail_start_idx = max(1, ti - tail_length);
    set(h_payload_tail, 'XData', eta_traj(tail_start_idx:ti, payload_indices(1)), ...
                        'YData', eta_traj(tail_start_idx:ti, payload_indices(2)), ...
                        'ZData', eta_traj(tail_start_idx:ti, payload_indices(3)));
    ...
    % pause(0.05); % Uncomment for slower playback
end
...
%% --- Helper Functions (Shared with other animation scripts) ---
